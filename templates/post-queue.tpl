...
<div class="btn-toolbar justify-content-end">
	...
	<div class="btn-group bottom-sheet" component="post-queue/bulk-actions">
		<button type="button" class="btn btn-ghost btn-sm dropdown-toggle" data-bs-toggle="dropdown" autocomplete="off" aria-haspopup="true" aria-expanded="false">
			<i class="fa fa-clone"></i><span class="fw-semibold"> {{tx("post-queue:bulk-actions")}}</span>
		</button>
		<ul class="dropdown-menu p-1 text-sm dropdown-menu-end">
			{{{ if canAccept }}}
			<li><a class="dropdown-item rounded-1" href="#" data-action="accept-all">{{tx("post-queue:accept-all")}}</a></li>
			<li><a class="dropdown-item rounded-1" href="#" data-action="accept-selected">{{tx("post-queue:accept-selected")}}</a></li>
			<li class="dropdown-divider"></li>
			<li><a class="dropdown-item rounded-1" href="#" data-action="reject-all">{{tx("post-queue:reject-all")}}</a></li>
			<li><a class="dropdown-item rounded-1" href="#" data-action="reject-selected">{{tx("post-queue:reject-selected")}}</a></li>
			{{{ else }}}
			<li><a class="dropdown-item rounded-1" href="#" data-action="reject-all">{{tx("post-queue:remove-all")}}</a></li>
			<li><a class="dropdown-item rounded-1" href="#" data-action="reject-selected">{{tx("post-queue:remove-selected")}}</a></li>
			{{{ end }}}
		</ul>
	</div>
	{{{ end }}}
				...
				<ul class="list-unstyled ps-0 mb-0 border-end h-100">
					<li class="card-body border-bottom position-relative">
						...
						<label for="{./id}" class="small stretched-link">
							{{{ if posts.data.tid }}}{{tx("post-queue:reply")}}{{{ else }}}{{tx("post-queue:topic")}}{{{ end }}}
						</label>
					</li>
					<li class="card-body d-flex flex-column gap-1 border-bottom">
						<div class="d-flex text-xs fw-semibold align-items-center">
							...
							<div class="ms-auto btn-group bottom-sheet">
								<button href="#" class="btn btn-outline-sm text-xs dropdown-toggle" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{tx("global:actions")}}</button>
								<ul class="dropdown-menu p-1 text-sm">
									{{{ if privileges.view:users:info }}}
									<li><a class="dropdown-item rounded-1" href="{config.relative_path}/user/{./user.userslug}/info">{{tx("user:account-info")}}</a></li>
									{{{ end }}}
									{{{ if privileges.ban }}}
					...
					<li class="card-body">
						...
						<div class="row row-cols-2 g-1">
							...
							{{{ if !posts.data.tid }}}
							<div class="col d-grid">
								<button class="btn btn-light btn-sm" data-action="editTitle"><i class="fa fa-fw fa-edit"></i> {{tx("post-queue:title")}}</button>
							</div>
							{{{ end }}}
							<div class="col d-grid">
								<button class="btn btn-light btn-sm" data-action="editContent"><i class="fa fa-fw fa-edit"></i> {{tx("post-queue:content")}}</button>
							</div>
							{{{if posts.data.cid}}}
							<div class="col d-grid">
								<button class="btn btn-light btn-sm" data-action="editCategory"><i class="fa fa-fw fa-edit"></i> {{tx("post-queue:category")}}</button>
							</div>
							{{{ end }}}
							<div class="col d-grid">
								<button class="btn btn-light btn-sm" data-action="notify"><i class="fa fa-fw fa-bell-o"></i> {{tx("post-queue:notify")}}</button>
							</div>
						</div>
						{{{ else }}}
