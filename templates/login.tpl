					...
					<form class="d-flex flex-column gap-3" role="form" method="post" id="login-form">
						...
						<div class="mb-2 d-flex flex-column gap-2">
							<div class="d-flex justify-content-between align-items-center">
								...
								<a id="reset-link" tabindex="-1" class="text-sm text-reset text-decoration-underline" href="{config.relative_path}/reset">{{tx("login:forgot-password")}}</a>
								{{{ end }}}
							</div>
							<div>
								<input class="form-control" type="password" placeholder="{{tx("user:password")}}" name="password" id="password" {{{ if username }}}autocomplete="off"{{{ end }}}/>
						...
						<div class="mb-2">
							<div class="checkbox">
								<label>
									<input type="checkbox" name="remember" id="remember" checked /> {{tx("login:remember-me")}}
								</label>
							</div>
						</div>
						{{{each loginFormEntry}}}
						...
						<span class="text-sm">{{tx("login:dont-have-account")}}</span>
