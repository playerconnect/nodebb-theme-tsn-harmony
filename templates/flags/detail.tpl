...
<div class="d-flex flex-column flex-md-row">
	<div class="flex-shrink-0 d-flex flex-column gap-3 border-end-md text-sm mb-3 pe-4" style="flex-basis: 240px !important;">
		<div class="d-grid gap-1">
			...
			<div class="btn-group dropend" data-uid="{target.uid}">
				<button type="button" class="btn btn-ghost border btn-sm justify-content-start dropdown-toggle" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
					...
					{{tx("flags:flagged-user")}}
				</button>
				<ul class="dropdown-menu p-1 text-sm">
					<li><a class="dropdown-item rounded-1" href="{config.relative_path}/uid/{target.uid}">{{tx("flags:view-profile")}}</a></li>
					{{{ if !config.disableChat }}}
						<li><a class="dropdown-item rounded-1" href="#" data-action="chat">{{tx("flags:start-new-chat")}}</a></li>
					{{{ end }}}
					<li class="dropdown-divider"></li>
					{{{ if privileges.ban }}}
					<li class="{{{ if target.user.banned }}}hidden{{{ end }}}"><a class="dropdown-item rounded-1" href="#" data-action="ban">{{tx("user:ban-account")}}</a></li>
					<li class="{{{ if !target.user.banned }}}hidden{{{ end }}}"><a class="dropdown-item rounded-1" href="#" data-action="unban">{{tx("user:unban-account")}}</a></li>
					{{{ end }}}
					{{{ if privileges.mute}}}
					<li class="{{{ if target.user.muted }}}hidden{{{ end }}}"><a class="dropdown-item rounded-1" href="#" data-action="mute">{{tx("user:mute-account")}}</a></li>
					<li class="{{{ if !target.user.muted }}}hidden{{{ end }}}"><a class="dropdown-item rounded-1" href="#" data-action="unmute">{{tx("user:unmute-account")}}</a></li>
					{{{ end }}}
					{{{ if privileges.admin:users }}}
					<li><a class="dropdown-item rounded-1" href="#" data-action="delete-account">{{tx("user:delete-account-as-admin")}}</a></li>
					<li><a class="dropdown-item rounded-1" href="#" data-action="delete-content">{{tx("user:delete-content")}}</a></li>
					<li><a class="dropdown-item rounded-1" href="#" data-action="delete-all">{{tx("user:delete-all")}}</a></li>
					{{{ end }}}
				</ul>
			</div>
			{{{ end }}}
			...
			<a class="btn btn-ghost border btn-sm justify-content-start" href="#" data-action="assign">
				...
				{{tx("flags:assign-to-me")}}
			</a>

			{{{ if type_bool.post }}}
			{{{ if !target.deleted}}}
			<a class="d-flex gap-2 align-items-center btn btn-sm btn-outline-danger border border-secondary-subtle text-start" href="#" data-action="delete-post"><i class="fa fa-fw fa-trash"></i> {{tx("flags:delete-post")}}</a>
			{{{ else }}}
			...
			<a class="d-flex gap-2 align-items-center btn btn-sm btn-outline-success border border-secondary-subtle text-start" href="#" data-action="restore-post"><i class="fa fa-fw fa-reply"></i><i class="fa fa-trash"></i> {{tx("flags:restore-post")}}</a>
			{{{ end }}}
			{{{ end }}}
		</div>
		...
		<form class="d-flex flex-column gap-3" id="attributes">
			<div>
				<label class="text-muted fw-semibold" for="state">{{tx("flags:state")}}</label>
				<select class="form-select form-select-sm" id="state" name="state" disabled>
					<option value="open">{{tx("flags:state-open")}}</option>
					<option value="wip">{{tx("flags:state-wip")}}</option>
					<option value="resolved">{{tx("flags:state-resolved")}}</option>
					<option value="rejected">{{tx("flags:state-rejected")}}</option>
				</select>
			</div>
			<div>
				<label class="text-muted fw-semibold" for="assignee">{{tx("flags:assignee")}}</label>
				<select class="form-control form-control-sm" id="assignee" name="assignee" disabled>
					<option value="">{{tx("flags:no-assignee")}}</option>
					{{{each assignees}}}
			...
			<div class="d-grid">
				<button type="button" class="btn btn-primary" data-action="update">{{tx("flags:update")}}</button>
			</div>
		</form>

		<div class="overflow-auto" component="flag/history" style="max-height: 30rem;">
			<h2 class="h6 fw-bold">{{tx("flags:history")}}</h2>
			{{{ if !history.length }}}
			<div class="alert alert-success text-center">{{tx("flags:no-history")}}</div>
			{{{ end }}}
			{{{ each history }}}
	...
	<div class="flex-grow-1 ps-md-2 ps-lg-5" style="min-width:0;">
		<div class="d-flex flex-column gap-4">
			...
			<div component="flag/content" class="d-flex flex-column gap-1 pb-3 border-bottom">
				...
				{{{ if type_bool.empty }}}
				<div class="alert alert-warning" role="alert">{{tx("flags:target-purged")}}</div>
				{{{ end }}}
			</div>
			<div class="flag/reports" class="pb-4 border-bottom">
				<h2 class="h6 fw-bold">{{tx("flags:reports")}}</h2>
			...
			<div class="pb-4 border-bottom">
				<div class="d-flex align-items-center">
					<h2 class="h6 fw-bold me-auto mb-0">{{tx("flags:notes")}}</h2>
					<button class="btn btn-ghost border" data-action="addEditNote">{{tx("flags:add-note")}}</button>
				</div>
				<ul component="flag/notes" class="list-unstyled mt-4">
					{{{ if !notes.length }}}
					<em>{{tx("flags:no-notes")}}</em>
					{{{ end }}}
					{{{ each notes }}}
