	...
	<div class="d-flex flex-column flex-md-row gap-2 w-100 pb-4 mb-4 mt-2 border-bottom">
		...
		<div class="d-flex flex-column flex-md-row mt-1 justify-content-between w-100 gap-2">
			<div class="d-flex flex-grow-1 flex-row gap-2">
				<div class="d-flex flex-column gap-1">
					...
					<div class="text-xm text-muted">
						{{{ if banned_until }}}
						{{tx("user:info.banned-until", banned_until_readable)}}
						{{{ else }}}
			...
			<div class="flex-shrink-0 d-flex gap-1 align-self-stretch align-self-md-start justify-content-end">
				{{{ if loggedIn }}}
				{{{ if !isSelf }}}
				<a component="account/unfollow" href="#" class="btn btn-info flex-fill{{{ if !isFollowing }}} hide{{{ end }}}">{{tx("user:unfollow")}}</a>
				<a component="account/follow" href="#" class="btn btn-primary flex-fill{{{ if isFollowing }}} hide{{{ end }}}">{{tx("user:follow")}}</a>
				{{{ end }}}
				{{{ end }}}
				...
				<div class="btn-group flex-fill">
					...
					<ul class="dropdown-menu dropdown-menu-end p-1 text-sm" role="menu">
						<li><a class="dropdown-item rounded-1" href="#" component="account/new-chat">{{tx("user:new-chat-with", username)}}</a></li>
					</ul>
					{{{ end }}}
				</div>
				{{{ end }}}
