...
<div style="max-height: 500px;overflow: auto;">
	...
	{{{ if !group.pending.length }}}
	<div class="alert alert-info">{{tx("groups:pending.none")}}</div>
	{{{ end }}}
	{{{each group.pending}}}
	<tr data-uid="{group.pending.uid}" class="align-middle">
		<td class="member-name p-2 d-flex align-items-center justify-content-between">
			...
			<div class="d-flex gap-2">
				<button class="btn btn-danger btn-sm" data-action="reject">{{tx("groups:pending.reject")}}</a></li>
				<button class="btn btn-success btn-sm" data-action="accept">{{tx("groups:pending.accept")}}</a></li>
			</div>
		</td>
	</tr>
	{{{end}}}
