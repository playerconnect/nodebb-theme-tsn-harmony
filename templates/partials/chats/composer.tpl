<div component="chat/composer" class="d-flex flex-column border-top pt-2 align-items-start">
	...
	<div class="w-100 flex-grow-1 flex-nowrap position-relative d-flex rounded-2 border border-secondary p-1 align-items-end">
		<button component="chat/upload/button" class="btn btn-ghost btn-sm px-2" type="button" title="{{tx("global:upload")}}" data-bs-toggle="tooltip"><i class="fa fa-fw fa-upload"></i></button>
		<div class="flex-grow-1 align-self-center">
			<textarea component="chat/input" placeholder="{{tx("modules:chat.placeholder.mobile")}}" class="bg-transparent text-body form-control chat-input mousetrap rounded-0 border-0 shadow-none px-1 py-0" style="min-height: 1.5rem;height:0;max-height:30vh;resize:none;"></textarea>
		</div>
		<div class="d-flex gap-1">
			{{{ each composerActions }}}
			<button data-action="{./action}" class="btn btn-ghost btn-sm px-2 {./class}" type="button" title="{{tx(./title)}}" data-bs-toggle="tooltip"><i class="fa {./icon}"></i></button>
			{{{ end }}}
			<button class="btn btn-ghost btn-sm px-2" type="button" data-action="send" title="{{tx("modules:chat.send")}}" data-bs-toggle="tooltip"><i class="fa fa-fw fa-paper-plane text-primary"></i></button>
		</div>
	</div>
